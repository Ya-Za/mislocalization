...
function DP = getDisplacement1()
    ...
    for t = 1:nt
        ...
        DP(t, :) = getDP1(W(:, idx{t}, t),  W(:, idx{t}, t), width_, height_);
    end
    toc();
...
function dp = getDP1(w1, w2, width, height)
...
function DP = getDisplacement2()
    ...
    for t = 1:nt
        ...
        DP(t, :, :, :) = getDP2(W(:, idx{t}, t),  W(:, idx{t}, t), width, height);
    end
    toc();
...
function dp = getDP2(w1, w2, width, height)
    width_ = width - 4;
    height_ = height - 4;
...
function DP = getDisplacement3()
    ...
    for t = 1:nt
        fprintf('Time: %d\n', times(t));
        % DP(t, :, :, :) = getDP3(fixW,  W(:, idx{t}, t), width, height);
        DP(t, :, :, :) = getDP3(W(:, idx{t}, t),  W(:, idx{t}, t), width, height);
    end
    toc();
...
function dp = getDP3(w1, w2, width, height)
    width_ = width - 2;
    height_ = height - 2;
    dp = zeros(width_, height_, 9);
